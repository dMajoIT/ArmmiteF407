...
void MIPS16 cmd_list(void) {
    ...
    if((p = checkstring(cmdline, "ALL"))) {
    	...
    	m=0;
		char** c=GetTempMemory((CommandTableSize+5)*sizeof(*c)+(CommandTableSize+5)*16);
		for(i=0;i<CommandTableSize+5;i++){
				c[m]= (char *)((int)c + sizeof(char *) * (CommandTableSize+5) + m*16);
...
search_again:
	while(*p != 0xff) {
		...
		if(*p == 0) error("No DATA to read");				        // end of the program and we still need more data
	...
	if(!*p || *p == '\'') { CurrentLinePtr = lineptr; error("No DATA to read"); }
		...
		while(vidx < vcnt) {
			...
			if(vtype[vidx] & T_STR) {
				...
				*((long long int *)vtbl[vidx]) = getinteger(argv[i]);// much easier if integer variable
			else
				*((MMFLOAT *)vtbl[vidx]) = getnumber(argv[i]);		// same for numeric variable

			NextData++;
			dnbr++;
			vidx++;
			i++;
			if(i < argc && *argv[i] != ',') error("Syntax");
			CurrentLinePtr = x;
			i++;
...
void cmd_erase(void) {
	...
	for(i = 0; i < argc; i += 2) {
        ...
        while(!isnamechar((uint8_t)p[strlen(p) - 1])) p[strlen(p) - 1] = 0;
