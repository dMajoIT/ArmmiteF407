...
void MIPS16 InitDisplaySPI(int fullinit) {
	...
	// Allow ILI9481,ILI9341,ILI9431_I,ST7735,ILI9888,ILI9486,ST7789,GA9A01
    ...
    if(fullinit) {
        ...
        OpenSpiChannel();
        // Option.SSDspeed is used to record the SPI panel attributes for use in the later functions.
    ...
    switch(Option.DISPLAY_TYPE) {
         case GC9A01:
        	  Option.SSDspeed=2;  //B0=ReadBuffer B1=RGB565
        	  DisplayHRes = 240;
        	  DisplayVRes = 240;
        	  ResetController();
         ...
         case ILI9488:
        	   Option.SSDspeed=1;  //B0=ReadBuffer B1=RGB565
               DisplayHRes = 480;
               DisplayVRes = 320;
               ResetController();
         ...
         case ILI9341:
         case ILI9341_I:
    	       Option.SSDspeed=3;  //B0=ReadBuffer B1=RGB565
		       DisplayHRes = 320;
		       DisplayVRes = 240;
		       ResetController();
         ...
         case ST7789:
        	 Option.SSDspeed=2;  //B0=ReadBuffer B1=RGB565  //2
        	 DisplayHRes = 240;
        	 DisplayVRes = 240;
        	 ResetController();
    ...
    if(Option.DISPLAY_TYPE!=ST7789 && Option.DISPLAY_TYPE!=ST7735  ){
        switch(Option.DISPLAY_ORIENTATION) {
    ...
    if(Option.SSDspeed & 0x1)ReadBuffer = ReadBufferSPI;
    //save the options
    SaveOptions();
...
void DrawBufferSPI(int x1, int y1, int x2, int y2, char* p) {
    ...
    if(Option.SSDspeed & 0x2)rgb565=1;
    // make sure the coordinates are kept within the display area
    if(x2 <= x1) { t = x1; x1 = x2; x2 = t; }
    if(y2 <= y1) { t = y1; y1 = y2; y2 = t; }
    if(x1 < 0) x1 = 0;
...
void DrawBitmapSPI(int x1, int y1, int width, int height, int scale, int fc, int bc, unsigned char *bitmap){
   ...
   // if(Option.DISPLAY_TYPE == ILI9341 || Option.DISPLAY_TYPE == ILI9481 || Option.DISPLAY_TYPE == ST7789)rgb565=1;
    if(Option.SSDspeed & 0x2)rgb565=1;
    if(( bc == -1) && !(Option.SSDspeed & 0x1))bc=0xFFFFFF;
