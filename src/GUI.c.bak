...
//        DoCallback(InvokingCtrl, "Open");
    ...
    if(mode == GUI_KEY_CANCEL || (is_alpha && KeyDown == 11) || (!is_alpha && KeyAltShift && KeyDown == 1)) {        // the user has hit the cancel key
...
void cmd_backlight(void) {
	getargs(&cmdline,3,",");
	if(argc==3){
		if(getinteger(argv[2])){
			htim1.Instance->CCR3=1000-getint(argv[0], 0, 100)*10;
		} else {
			htim1.Instance->CCR3=getint(argv[0], 0, 100)*10;
		}
	} else {
		if(HRes != 0 && ((Option.DISPLAY_TYPE >= SSD1963_4 && Option.DISPLAY_TYPE<=SSD_PANEL) || Option.DISPLAY_TYPE==IPS_4_16)){
			if(Option.DISPLAY_TYPE!=IPS_4_16 && Option.DISPLAY_TYPE!=ILI9341_16)SetBacklightSSD1963(getint(argv[0], 0, 100));
			htim1.Instance->CCR3=1000-getint(argv[0], 0, 100)*10;
		} else {
			htim1.Instance->CCR3=getint(argv[0], 0, 100)*10;
		}
...
void ResetGUI(void) {
    ...
    CurrentPages = 1;
    for(i = 1; i < Option.MaxCtrls; i++) {
        Ctrl[i].state = Ctrl[i].type = Ctrl[i].font = 0;
