...
//    		PIntComma(Option.TOUCH_SWAPXY);
    ...
    if((p = checkstring(cmdline, "RESET"))) {
        if((checkstring(p, "LCDPANEL"))) {
            InitDisplaySPI(0);
	if(((Option.DISPLAY_TYPE >= SSD1963_4 && Option.DISPLAY_TYPE <= SSD_PANEL) || Option.DISPLAY_TYPE==ILI9341_16 || Option.DISPLAY_TYPE==IPS_4_16 ) )  InitDisplaySSD(0);
            if(Option.TOUCH_CS) {
